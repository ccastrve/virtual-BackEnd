...
fecha_cursada		year not null, -- por si acaso el alumno repita
constraint fk_alumno foreign key (id_alumno) references alumnos (id),
constraint fk_grado foreign key (id_grado) references grados (id)
);



# podmeos hacer diagramas ER en file-new model o tambi{en en database-ingeniera inversa
-- faker python libreria para generar data para simulacion. --> https://faker.readthedocs.io/en/master/
